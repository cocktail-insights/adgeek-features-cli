...
const update = (collection, query, doc) => (
  new Promise((resolve, reject) => {
    collection.updateMany(query, doc, (e, r) => {
      if (e) {
        reject(e);
      } else {
        resolve(r.result.ok);
...
const insertUnique = async (collection, query, doc) => {
  ...
  if (!existingDoc) {
    ...
    if (yesOrNo === 'Y') {
      newDoc = await update(collection, query, { $set: doc });
      operationCompleted = true;
    }
  }
...
export default {
  new() {
    const run = async () => {
      ...
      const selectedPackage = await prompt('package (default: all): ');
      ...
      try {
        const { featureResult } = await insertUnique(collection, { codename: codeName }, doc);

        // create query for updating package
        const packageQuery = selectedPackage ? { name: selectedPackage.toLowerCase() } : {};
        const { packageResult } = await update(packagesCollection, packageQuery, {
          $addToSet: {
            featureList: codeName,
          },
        });

        console.log('Result from unique insertion', featureResult.ok, packageResult.ok);
      } catch (err) {
  ...
  set(args = {}) {
    const run = async () => {
      ...
      try {
        const result = await update(collection, { codename: parseCodeName(codename) }, doc);
